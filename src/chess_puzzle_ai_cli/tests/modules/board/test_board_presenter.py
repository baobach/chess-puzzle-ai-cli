...
from chess_puzzle_ai_cli.modules.common import get_piece_unicode_symbol
from chess_puzzle_ai_cli.utils.config import GameConfig
...
def presenter(model: BoardModel, game_config: GameConfig, monkeypatch):
    monkeypatch.setattr('chess_puzzle_ai_cli.modules.board.board_presenter.game_config', game_config)
    return BoardPresenter(model)


@pytest.fixture
def game_config(monkeypatch):
    # Patch the global game_config instance with a test-specific instance
    test_game_config = GameConfig("unit_test_config.ini")
    monkeypatch.setattr('chess_puzzle_ai_cli.utils.config.game_config', test_game_config)
    yield test_game_config
    remove(test_game_config.full_filename)
